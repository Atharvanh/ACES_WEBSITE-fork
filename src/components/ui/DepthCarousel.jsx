          ...
          >
            ...
            {item.title && (
              <div className="depth-carousel__meta">
                {item.category && (
                  <span className="text-[10px] font-bold uppercase tracking-wider bg-primary text-white px-2 py-0.5 rounded-[4px] w-max">
                    {item.category}
                  </span>
                )}
                <span className="text-sm font-extrabold line-clamp-1 font-display">{item.title}</span>
                {item.year && <span className="text-[11px] text-gray-300 font-mono">{item.year} • {item.location || 'DIT Pune'}</span>}
              </div>
            )}
          </div>
        ))}
