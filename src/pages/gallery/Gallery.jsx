...
export default function Gallery({ embedded = false }) {
  ...
  return (
    <div id="gallery" className="w-full bg-white text-dark-overlay">
      ...
      <section 
        ref={heroRef}
        className={`relative ${embedded ? 'h-[400px] sm:h-[480px]' : 'h-[440px] sm:h-[520px] mt-4'} overflow-hidden flex items-center justify-center`}
      >
        {/* Ambient Glow */}
        <div 
        ...
        <div className="absolute inset-0 w-full h-full overflow-hidden pointer-events-none opacity-55 select-none">
              ...
              <div className="marquee-container animate-marquee-sync flex flex-col gap-3 sm:gap-4">
                {[...col1, ...col2].map((img, idx) => (
                  <div key={`sync1-${idx}`} className="w-full aspect-[4/3] rounded-[4px] overflow-hidden bg-light-tint shadow-sm">
                    <img src={img.url} alt={img.alt} className="w-full h-full object-cover grayscale-[60%] opacity-85" />
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="marquee-container animate-marquee-col1 flex flex-col gap-4">
                {col1.map((img, idx) => (
                  <div key={`col1-${idx}`} className="w-full aspect-[4/3] rounded-[4px] overflow-hidden bg-light-tint shadow-sm">
                    <img src={img.url} alt={img.alt} className="w-full h-full object-cover grayscale-[50%] hover:grayscale-0 transition-all duration-300 opacity-80" />
                  </div>
                ))}
              </div>
            </div>
